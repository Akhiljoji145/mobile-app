...
const ManagementProfileScreen = ({ user, onLogout, navigation }) => {
            ...
            <View style={[styles.header, { backgroundColor: theme.headerBg }]}>
                <View style={styles.headerTop}>
                    <View>
                        ...
                        <Text style={styles.headerSubtitle}>View and manage settings</Text>
                    </View>
                </View>
                <Ionicons name="person" size={100} color="rgba(255,255,255,0.05)" style={styles.headerIcon} />
            </View>
                ...
                <View style={styles.profileHeader}>
                    <View style={styles.avatarContainer}>
                        <View style={[styles.avatarPlaceholder, { backgroundColor: theme.primary || '#3498DB' }]}>
                            <Text style={styles.avatarText}>{user?.username?.charAt(0).toUpperCase() || 'M'}</Text>
                        </View>
                    </View>
                    <Text style={[styles.name, { color: theme.text }]}>
                        {user?.first_name ? `${user.first_name} ${user.last_name || ''}` : user?.username || 'Management User'}
                    </Text>
                    <Text style={[styles.role, { color: theme.subtext }]}>Management</Text>
                </View>
