        ...
        return (
            ...
            >
                ...
                <View style={styles.cardContent}>
                    ...
                    <Text style={[styles.cardValue, { color: theme.text }]}>{String(value || '-')}</Text>
                    {subtext ? <Text style={[styles.cardSubtitle, { color: color }]}>{String(subtext)}</Text> : null}
                </View>
            </Container>
        );
    };
            ...
            >
                    ...
                    {data.bus ? (
                        <View style={styles.busInfoContainer}>
                            <View style={styles.busDetail}>
                                ...
                                <Text style={[styles.value, { color: theme.text }]}>{String(data.bus.number || '-')}</Text>
                            </View>
                            <View style={styles.busDetail}>
                                <Text style={[styles.label, { color: theme.subtext }]}>Plate Number</Text>
                                <Text style={[styles.value, { color: theme.text }]}>{String(data.bus.plate || '-')}</Text>
                            </View>
                            <View style={styles.busDetail}>
                                <Text style={[styles.label, { color: theme.subtext }]}>Driver</Text>
                                <Text style={[styles.value, { color: theme.text }]}>{String(data.bus.driver_name || '-')}</Text>
                            </View>
                        </View>
                    ) : (
                ...
                {(!data.notifications || data.notifications.length === 0) ? (
                    ...
                    data.notifications.map((notif, index) => (
                        <View key={notif.id || index} style={[styles.notifCard, { backgroundColor: theme.card }]}>
                            <View style={styles.notifHeader}>
                                <Text style={[styles.notifTitle, { color: theme.text }]}>{String(notif.title || '')}</Text>
                                <Text style={[styles.notifTime, { color: theme.subtext }]}>{String(notif.time || '')}</Text>
                            </View>
                            <Text style={[styles.notifMessage, { color: theme.textSecondary }]}>{String(notif.message || '')}</Text>
                        </View>
                    ))
                )}
