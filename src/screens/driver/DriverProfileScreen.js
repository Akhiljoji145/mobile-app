...
const DriverProfileScreen = ({ user, onLogout, navigation }) => {
        ...
        >
            <ScrollView contentContainerStyle={styles.scrollContent}>
                ...
                <View style={styles.header}>
                    <View style={styles.avatarContainer}>
                        <View style={[styles.avatarPlaceholder, { backgroundColor: theme.primary || '#3498DB' }]}>
                            <Text style={styles.avatarText}>{user?.username?.charAt(0).toUpperCase() || 'D'}</Text>
                        </View>
                    </View>
                    <Text style={[styles.name, { color: theme.text }]}>
                        {user?.first_name ? `${user.first_name} ${user.last_name || ''}` : user?.username || 'Driver'}
                    </Text>
                    <Text style={[styles.role, { color: theme.subtext }]}>Driver</Text>
                </View>
                ...
                <View style={[styles.card, { backgroundColor: theme.card }]}>
                    ...
                    <InfoRow label="Phone" value={user?.phone_number || user?.phone} icon="call-outline" />
                </View>
