...
const DriverTripScreen = ({ navigation }) => {
        ...
        const fetchStatus = async () => {
            try {
                ...
                if (tripData && tripData.status === 'Ongoing') {
                    ...
                    // Resume tracking if not already doing so
                    if (!locationSubscription) {
                ...
                console.log('Error fetching current trip status', error);
            }
        };
        fetchStatus();
    }, []);
    ...
    const startTrip = async () => {
        ...
        try {
            ...
            const errorMessage = error.response?.data?.error || 'Failed to start trip. Please try again.';
    ...
    const endTrip = async () => {
        ...
        try {
            ...
            const errorMessage = error.response?.data?.error || 'Failed to end trip properly.';
            Alert.alert('Error', errorMessage);
        } finally {
            setLoading(false);
        }
